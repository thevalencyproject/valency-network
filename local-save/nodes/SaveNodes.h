...
struct SaveNodes {
    ...
    std::vector<Position4D> nodes;     // The nodes vector that is stored in the nodes.vlnc file
    ...
    void read(std::vector<Position4D>* n, std::string filepath);    // Reads the nodes - stores nodes at pointer
    void save();                                                    // Saves the nodes to the filepath (reverse of reading the file)
    void save(std::vector<Position4D>* n, std::string filepath);    // Saves the nodes at the input pointer to the inputted filepath
};
